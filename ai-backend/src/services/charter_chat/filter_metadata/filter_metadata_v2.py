...
from src.services.charter_chat.charter_chat_schema import (
    FilterMetaDataLLMResponseV2,
    QueryMetadata,
    QueryPayload,
)
from src.services.charter_chat.filter_metadata.filter_metadata import filter_charter_metrics
...
async def filter_data_entities_v2(data_entities, processed_prompt, model_type: LLMModelType = "large"):
    ...
    async def process_entity(entity):
        ...
        response: FilterMetaDataLLMResponseV2 = await LLMClientService.messages_with_instruction(
...
async def filter_charter_examples_v2(charter_examples, processed_prompt, model_type: LLMModelType = "large"):
...
async def filter_charter_metric_examples_v2(
    charter_metric_examples, processed_prompt, model_type: LLMModelType = "large"
):
    logger.info(f"Filtering {len(charter_metric_examples)} charter metric examples")

    async def process_example(example):
        logger.debug(f"Processing charter metric example: ID {example.id}")
        ...
        try:
            if response.relevance_score >= 2:
                logger.info(f"Charter metric example ID {example.id} is relevant")
                return example
            else:
                logger.debug(f"Charter metric example ID {example.id} is not relevant")
                return None
        except json.JSONDecodeError:
            logger.error(f"Failed to parse JSON response for charter metric example {example.id}")
            return None
...
async def filter_metadata_using_llm_v2(
    processed_query: QueryPayload, vector_search_result: QueryMetadata, model_type: LLMModelType = "large"
) -> QueryMetadata:
    processed_prompt = processed_query.prompt

    filtered_results = await asyncio.gather(
        filter_data_entities_v2(vector_search_result.data_entities, processed_prompt, model_type),
        filter_charter_examples_v2(vector_search_result.charter_examples, processed_prompt, model_type),
        filter_charter_metrics(vector_search_result.charter_metrics, processed_prompt, model_type),
        filter_charter_metric_examples_v2(vector_search_result.charter_metric_examples, processed_prompt, model_type),
    )
